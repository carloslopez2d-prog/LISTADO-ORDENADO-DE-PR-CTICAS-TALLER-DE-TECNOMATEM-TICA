...
Y = [1 / (1 + np.exp(-x)) for x in X]  # Función objetivo f(x) = x^2
...
print("Entrenando el Perceptrón Cuántico (Cross-Entropy)...")
...
for epoch in range(epochs):
   ...
   for x, y in zip(X, Y):
       # Forward pass (Circuito cuántico)
       z = (w * x + b) / 2.0
       y_hat = math.sin(z)**2
       current_preds.append(y_hat)
      
       y_hat_safe = max(min(y_hat, 1 - epsilon), epsilon)
      
       # Función de pérdida (Cross Entropy)
       loss = - (y * math.log(y_hat_safe) + (1 - y) * math.log(1 - y_hat_safe))
       total_loss += loss
      
       # Backward pass (Derivadas analíticas exactas)
       dL_dyhat = (y_hat_safe - y) / (y_hat_safe * (1 - y_hat_safe))
...
ax.plot(X, Y, label="$Sigmoide$ (Objetivo)", color="C0", linewidth=2.5)
...
def update(frame_idx):
   ...
   ax.set_title(f"Época: {epoch:03d} / {epochs}\nCross-Entropy Loss: {loss:.4f} | w: {current_w:.4f} | b: {current_b:.4f}")
   return line,
...
# Buscar la ruta del escritorio (funciona en Windows, Mac y Linux)
desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
# Si el sistema está en español y "Desktop" no existe, probamos "Escritorio"
if not os.path.exists(desktop_path):
   desktop_path = os.path.join(os.path.expanduser("~"), "Escritorio")


gif_filepath = r"C:\Users\pablo\OneDrive\Escritorio\ajuste_perceptron_cuantico.gif"


# Guardar el GIF (a 30 fps, el GIF durará unos 26 segundos para 800 frames)
print(f"Guardando GIF en: {gif_filepath} ... (Esto puede tardar un poco)")
...
# --- 2. GENERACIÓN DE DATOS ---
X = np.linspace(-1, 1, N)
Y = 1 / (1 + np.exp(-X))   # Función objetivo: sigmoide
...
def update(frame_idx):
   preds, epoch, loss, current_w, current_b = history[frame_idx]
   line.set_ydata(preds)
   ax.set_title(f"Época: {epoch:03d} / {epochs}\nMSE Loss: {loss:.4f} | w: {current_w:.4f} | b: {current_b:.4f}")
   return line,
